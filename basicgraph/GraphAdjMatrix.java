...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		int[][] MultadjMatrix = new int[adjMatrix.length][adjMatrix.length];
		List<Integer> hopTwo = new ArrayList<Integer>();
		for (int i = 0; i < adjMatrix.length; i ++) {
			for (int j = 0; j < adjMatrix.length; j ++) {
				for(int k=0;k<adjMatrix.length;k++){
					MultadjMatrix[i][j]+=(adjMatrix[i][k]*adjMatrix[k][j]);
				    } 
			}
		}
		for(int i=0;i< adjMatrix.length; i++){
		if(MultadjMatrix[v][i]!=0){
			for(int k=0; k< MultadjMatrix[v][i]; k++){
			hopTwo.add(i);
			}
		}
		}
